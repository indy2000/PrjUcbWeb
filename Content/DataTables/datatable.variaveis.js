function padLeft(number, digits) {
  return (
    Array(Math.max(digits - String(number).length + 1, 0)).join(0) + number
  );
}

function RetornaDataHoraAtual() {
  var dNow = new Date();
  var localdate =
    padLeft(dNow.getDate().toString(), 2) +
    "/" +
    padLeft(dNow.getMonth() + 1, 2) +
    "/" +
    dNow.getFullYear() +
    " " +
    dNow.getHours() +
    ":" +
    dNow.getMinutes();
  return localdate;
}

var imgDataTable =
  "<img src='" +
  baseUrl +
  "/Images/loading/loading-spinner-grey.gif'> Processando</img>";

var traducaoDataTable = {
  sEmptyTable: "Nenhum registro encontrado",
  sInfo: "Mostrando de _START_ até _END_ de _TOTAL_ registros",
  sInfoEmpty: "Mostrando 0 até 0 de 0 registros",
  sInfoFiltered: "(Filtrados de _MAX_ registros)",
  sInfoPostFix: "",
  sInfoThousands: ".",
  sLengthMenu: "Exibir _MENU_ resultados por página",
  sLoadingRecords: "Carregando...",
  sProcessing: imgDataTable,
  sZeroRecords: "Nenhum registro encontrado",
  sSearch: "Pesquisar",
  oPaginate: {
    sNext: "Próximo",
    sPrevious: "Anterior",
    sFirst: "Primeiro",
    sLast: "Último"
  },
  oAria: {
    sSortAscending: ": Ordenar colunas de forma ascendente",
    sSortDescending: ": Ordenar colunas de forma descendente"
  }
};

var traducaoDataTableSimples = {
  sEmptyTable: "Nenhum registro encontrado",
  sInfo: "Mostrando _TOTAL_ registros",
  sInfoEmpty: "Mostrando 0 registros",
  sInfoFiltered: "(Filtrados de _MAX_ registros)",
  sInfoPostFix: "",
  sInfoThousands: ".",
  sLengthMenu: "Exibir _MENU_ resultados por página",
  sLoadingRecords: "Carregando...",
  sProcessing: imgDataTable,
  sZeroRecords: "Nenhum registro encontrado",
  sSearch: "Pesquisar",
  oPaginate: {
    sNext: "Próximo",
    sPrevious: "Anterior",
    sFirst: "Primeiro",
    sLast: "Último"
  },
  oAria: {
    sSortAscending: ": Ordenar colunas de forma ascendente",
    sSortDescending: ": Ordenar colunas de forma descendente"
  }
};

var oldExportAction = function(self, e, dt, button, config) {
  if (button[0].className.indexOf("buttons-excel") >= 0) {
    if ($.fn.dataTable.ext.buttons.excelHtml5.available(dt, config)) {
      $.fn.dataTable.ext.buttons.excelHtml5.action.call(
        self,
        e,
        dt,
        button,
        config
      );
    } else {
      $.fn.dataTable.ext.buttons.excelFlash.action.call(
        self,
        e,
        dt,
        button,
        config
      );
    }
  } else if (button[0].className.indexOf("buttons-copy") >= 0) {
    $.fn.dataTable.ext.buttons.copyHtml5.action(self, e, dt, button, config);
  } else if (button[0].className.indexOf("buttons-csv") >= 0) {
    if ($.fn.dataTable.ext.buttons.csvHtml5.available(dt, config)) {
      $.fn.dataTable.ext.buttons.csvHtml5.action(self, e, dt, button, config);
    } else {
      $.fn.dataTable.ext.buttons.csvFlash.action(self, e, dt, button, config);
    }
  } else if (button[0].className.indexOf("buttons-print") >= 0) {
    $.fn.dataTable.ext.buttons.print.action(self, e, dt, button, config);
  }
};

var newExportAction = function(e, dt, button, config) {
  var self = this;
  var oldStart = dt.settings()[0]._iDisplayStart;

  dt.one("preXhr", function(e, s, data) {
    // Just this once, load all data from the server...
    data.start = 0;
    data.length = 2147483647;

    dt.one("preDraw", function(e, settings) {
      // Call the original action function
      oldExportAction(self, e, dt, button, config);

      dt.one("preXhr", function(e, s, data) {
        // DataTables thinks the first item displayed is index 0, but we're not drawing that.
        // Set the property to what it was before exporting.
        settings._iDisplayStart = oldStart;
        data.start = oldStart;
      });

      // Reload the grid with the original page. Otherwise, API functions like table.cell(this) don't work properly.
      setTimeout(dt.ajax.reload, 0);

      // Prevent rendering of the full data to the DOM
      return false;
    });
  });

  // Requery the server with the new one-time export settings
  dt.ajax.reload();
};

const buttonsDataTable = [
  {
    text: "Copiar Selecionados",
    extend: "copyHtml5",
    className: "btn-sm",
    exportOptions: {
      modifier: {
        selected: null
      }
    },
    messageTop: "Emitido por " + username + " em " + RetornaDataHoraAtual(),
    bShowAll: true
  },
  {
    text: "CSV",
    extend: "csvHtml5",
    className: "btn-sm",
    oSelectorOpts: {
      page: "all"
    },
    bShowAll: true
  },
  {
    text: "Excel",
    extend: "excelHtml5",
    className: "btn-sm",
    exportOptions: {
      columns: "th:not(:first-child)",
      orthogonal: "export"
      //format: {
      //  body: function(data, row, column, node) {
      //    data = $("<p>" + data + "</p>").text();
      //        return $.isNumeric(data.replace(".", "").replace(",", ".")) ? data.replace(".", "").replace(",", ".") : data;
      //  }
      //}
    },
    oSelectorOpts: {
      page: "all"
    },
    action: newExportAction,
    bShowAll: true,
    sheet_name: "Sinapse"
  },
  {
    text: "PDF",
    extend: "pdfHtml5",
    className: "btn-sm",
    oSelectorOpts: {
      page: "all"
    },
    bShowAll: true
  },
  {
    text: "Imprimir",
    extend: "print",
    className: "btn-sm",
    oSelectorOpts: {
      page: "all"
    },
    bShowAll: true
  }
];

const buttonsDataTableSimples = [
  {
    extend: "excelHtml5",
    exportOptions: {
      orthogonal: "export",
      columns: "th:not(:first-child)"
    },
    messageTop: "Emitido por " + username + " em " + RetornaDataHoraAtual(),
    sheet_name: "Sinapse"
  }
];

function agruparDados(indexes, dadosListagem) {
  const dadosAgrupados = [];

  const [idxGrupo, idxSubGrupo] = indexes;

  const header = dadosListagem.shift();
  dadosAgrupados.push(header);

  const grupos = Array.from(
    new Set(dadosListagem.map(arr => arr[idxGrupo].text))
  );

  grupos.forEach(grupo => {
    let dadosGrupo = dadosListagem.filter(arr => arr[idxGrupo].text === grupo);

    if (idxSubGrupo !== undefined) {
      const dados = [];
      const subGrupos = Array.from(
        new Set(dadosGrupo.map(arr => arr[idxSubGrupo].text))
      );

      subGrupos.forEach(subgrupo => {
        const dadosSubGrupo = dadosGrupo.filter(
          arr => arr[idxSubGrupo].text == subgrupo
        );
          const descSubGrupo = [
              {
                  agrupamento: 1,
                  text: subgrupo || "Sem grupo",
                  style: "tableBodyOdd",
                  fillColor: "#DBDBDB"
              }
          ];

        for (let i = 1; i <= header.length - 1; i++) {
          descSubGrupo.push({
            text: "",
            style: "tableBodyOdd",
            fillColor: "#DBDBDB"
          });
        }

        dados.push(descSubGrupo, ...dadosSubGrupo);
      });

      dadosGrupo = dados;
    }

      const descGrupo = [
          {
              agrupamento: 1,
              text: grupo || "Sem grupo",
              style: "tableBodyOdd",
              fillColor: "#B8B8B8"
          }
      ];

    for (let i = 1; i <= header.length - 1; i++) {
      descGrupo.push({ text: "", style: "tableBodyOdd", fillColor: "#B8B8B8" });
    }

    dadosAgrupados.push(descGrupo, ...dadosGrupo);
  });

  return dadosAgrupados;
}

function GetMessageTop() {
  try {
    let filtrosStr = $("#divSubTitle")[0].innerText;
    let filtrosArr = filtrosStr.split("\n");

    if (filtrosStr.includes("Filtrado por:"))
      filtrosArr = filtrosArr.filter((value, index) => index !== 0);

    return `Filtrado por: ${filtrosArr.join("  -  ")}`;
  } catch (e) {
    console.log(e);
    return "";
  }
}

/**
 * Buttons de Exportação para Excel
 * @param {any} config title - Titulo
 * groupingIndex - index da coluna para agrupar
 * color - cor da fonte
 * fillColor - cor de fundo
 * fontSize - tamanho da fonte
 * sheet_name - nome do sheet
 * printcolumns - colunas a imprimir
 * messageBottom - mensagem no rodapé
 * messageTop - mensagem no topo (abaixo cabeçalho)
 * classNameBtnExcel - classe para o botão de impressao Excel
 * classNameBtnPDF - classe para o botão de impressao PDF
 * pdfCustomize - função para adicionar comportamento customizado na exportação para PDF
 * exportOptions - objeto de opções de exportação
 * header - function para sobrescrever o header
 * footer - function para sobrescrever o footer
 * @return {void}
 */
const buttonsDataTableRelatorio = function(config) {
  let title = config.title ? config.title : "Titulo";
  let color = config.color ? config.color : "white";
  let fillColor = config.fillColor ? config.fillColor : "#222D32";
  let fontSize = config.fontSize ? config.fontSize : 8;
  let hideFirstRow = config.hideFirstRow ? true : false;
  let pageSize = config.pageSize ? config.pageSize : "LEGAL";

  let retorno = [
    {
      extend: "excelHtml5",
      exportOptions: {
        orthogonal: "export"
      },
      sheet_name: "Sinapse",
      title: title,
      messageTop: GetMessageTop,
      oSelectorOpts: {
        page: "all"
      },
      bShowAll: true,
      customize: function(xlsx) {
        var sheet = xlsx.xl.worksheets["sheet1.xml"];

        $("row:first c", sheet).attr("s", "2");

        $($("row c", sheet)[1]).attr("s", "2");
        if (hideFirstRow) {
          $("row:first c", sheet).attr("s", "2");
        }

        //$('row* ', sheet).each(function (index) {
        //    if (index === 1) {
        //        $(this).attr('ht', 50);
        //        $(this).attr('customHeight', 1);

        //        //wrap text
        //        $(this).attr('s', '55');

        //        /*
        //        $(this).attr('s', '55'); //wrap

        //        var cellText = $('t', $(this));
        //        if (cellText.text() !== null) { //&& cellText.text().indexOf('~') > 0
        //            cellText.text(cellText.text());
        //            //cellText.text(cellText.text().replace(/~/g, "\n")); //insert stripped line break
        //        }*/
        //    }
        //});

        //Seletor Funcionou
        //$('row c[r^="A2"]', sheet).attr('ht', 160);

        // Loop over the cells in column `C`
        //$('row c[r^="C"]', sheet).each(function () {
        //    // Get the value
        //    if ($('is t', this).text() == 'New York') {
        //        $(this).attr('s', '20');
        //    }
        //});
      }
    },
    {
      extend: "pdfHtml5",
      orientation: "landscape",
      pageSize: pageSize,
      title: "",
      exportOptions: {
        modifier: {
          page: "all"
        },
        orthogonal: "export"
      },
      header: true,
      styles: {
        // style for printing PDF body
        fullWidth: {
          fontSize: 8,
          bold: true,
          alignment: "right",
          margin: [0, 0, 0, 0]
        }
      },
      oSelectorOpts: {
        page: "all"
      },
      bShowAll: true,
      customize: function(doc) {
        if (typeof config.pdfCustomize === "function") {
          config.pdfCustomize(doc);
        }

        if (config.groupingIndex !== undefined) {
          doc.content[0].table.body = agruparDados(
            config.groupingIndex,
            doc.content[0].table.body
          );
        }

        doc.styles.tableHeader.fontSize = 8;
        //doc.defaultStyle.font = 'Arial';
        doc.defaultStyle.fontSize = 8;
        doc.defaultStyle.alignment = "center";
        doc.styles.tableHeader = {
          alignment: "center",
          fontSize: fontSize,
          color: color,
          fillColor: fillColor
        };

        try {
          doc.content[0].table.widths = Array(
            doc.content[0].table.body[0].length + 1
          )
            .join("*")
            .split("");
        } catch (e) {
          console.log(e);
        }

        let subTitulo = () => {
          return $("#divSubTitle").length > 0 ? GetMessageTop() : "";
        };

        const imageSubtitle = {
          columns: [
            {
              width: "100%",
              table: {
                body: [
                  [
                    {
                      text: subTitulo(),
                      alignment: "left",
                      fontSize: 8,
                      margin: [0, 0, 0, 10]
                    }
                  ]
                ]
              },
              layout: "noBorders"
            }
          ]
        };
        doc.content.unshift(imageSubtitle);

        let logo =
          "data:image/png;base64,";
        if (logoCliente) logo = "data:image/jpg;base64," + logoCliente;

        const imageTitle = {
          columns: [
            {
              image: logo,
              margin: [10, 10, 10, 10]
            },
            {
              width: "70%",
              table: {
                body: [
                  [
                    {
                      text: title,
                      fontSize: 26,
                      alignment: "center"
                      //margin: [0, 0, 0, 0]
                    }
                  ]
                ]
              },
              layout: "noBorders"
            }
          ]
        };

        doc.content.unshift(imageTitle);

        //header
        if (config.header) doc["header"] = config.header;

        // Create a footer
        doc["footer"] = config.footer
          ? config.footer
          : function(page, pages) {
              return {
                columns: [
                  {
                    // This is the right column
                    alignment: "right",
                    text: [
                      "Página ",
                      { text: page.toString() },
                      " de ",
                      { text: pages.toString() }
                    ]
                  }
                ],
                margin: [10, 0]
              };
            };
      }
    }
  ];

  retorno.map((p, index) => {
    if (config.filename) {
      p.filename = config.filename;
    }

    if (config.sheet_name) {
      if (p.extend === "excelHtml5") {
        p.sheet_name = config.sheet_name;
      }
    }

    if (config.printcolumns) {
      p.exportOptions.columns = config.printcolumns; // Colunas visiveis
    }

    if (config.messageBottom) {
      p.messageBottom = config.messageBottom; // Mensagem no footer
    }

    if (config.messageTop) {
      p.messageTop = config.messageTop; // Mensagem no topo
    }

    if (config.classNameBtnExcel) {
      if (p.extend === "excelHtml5") {
        p.className = config.classNameBtnExcel; //Determina o nome da classe do botão Excel
      }
    }

    if (config.classNameBtnPDF) {
      if (p.extend === "pdfHtml5") {
        p.className = config.classNameBtnPDF; //Determina o nome da classe do botão PDF
      }
    }

    if (config.exportOptions) {
      p.exportOptions = config.exportOptions; // Injeta opções de Exportação
    }

    return p;
  });

  return retorno;
};

//
// Updates "Select all" control in a data table
//
function updateDataTableSelectAllCtrl(table) {
  var $table = table.table().node();
  var $chkbox_all = $('tbody input[type="checkbox"]', $table);
  var $chkbox_checked = $('tbody input[type="checkbox"]:checked', $table);
  var chkbox_select_all = $('thead input[name="select_all"]', $table).get(0);

  // If none of the checkboxes are checked
  if ($chkbox_checked.length === 0) {
    chkbox_select_all.checked = false;
    if ("indeterminate" in chkbox_select_all) {
      chkbox_select_all.indeterminate = false;
    }

    // If all of the checkboxes are checked
  } else if ($chkbox_checked.length === $chkbox_all.length) {
    chkbox_select_all.checked = true;
    if ("indeterminate" in chkbox_select_all) {
      chkbox_select_all.indeterminate = false;
    }

    // If some of the checkboxes are checked
  } else {
    chkbox_select_all.checked = true;
    if ("indeterminate" in chkbox_select_all) {
      chkbox_select_all.indeterminate = true;
    }
  }
}

var renderExportDinero = (data, type, row) => {
  let retorno;

  if (type === "export") {
    retorno = data.replace(".", "").replace(",", ".");
  } else retorno = data;

  return retorno;
};

const customizeExport = data => {
  for (var i = 0; i < data.body.length; i++) {
    for (var j = 0; j < data.body[i].length; j++) {
      data.body[i][j] = "\u200C" + data.body[i][j];
    }
  }
};

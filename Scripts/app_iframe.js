...
var App = function () {
    ...
    //Inicializar a altura da página de conteúdo do iframe
    var handleIframeContent = function () {
        var ht = $(window).height();//Obter a altura geral da janela do navegador；

        //var $footer = $(".main-footer");
        var $header = $(".main-header");
        var $tabs = $(".content-tabs");

        var height = App.getViewPort().height - 60;//$header.outerHeight()); // + 10 - $footer.outerHeight()
        //if ($tabs.is(":visible")) {
    ...
    //Inicializar a altura do componente de layout da página de conteúdo
    ...
    var requestFullScreen = function () {
        ...
        if (de.requestFullscreen) {
            ...
            App.alert({message: "O navegador não suporta tela cheia！", type: "danger"});
        }

    };

    var requestFullScreen2 = function (element) {
        // Julgue vários navegadores e encontre o caminho certo
    ...
    //Sair da tela cheia para determinar o tipo de navegador
    ...
    return {
        ...
        startPageLoading: function (options) {
            if (options && options.animate) {
                ...
                $('.page-loading').remove();
                $('body').append('<div class="page-loading"><img src="' + this.getGlobalImgPath() + 'loading/loading-spinner-grey.gif"/>&nbsp;&nbsp;<span>' + (options && options.message ? options.message : 'Carregando...') + '</span></div>');
            }
        },
...
function canRemoveTab(pageId) {
    return findTabTitle(pageId).size() > 0;
    //return findTabTitle(pageId).find('.fas-remove').size() > 0;
}
...
var addTabs = function (options) {
    ...
    //Determine se a guia desse ID já existe e, se ela não existir, crie uma nova.
    if (findTabPanel(pageId) === null) {

        //Crie um novo título de tabulação
        ...
        if (options.content) {
            //Se deseja especificar o conteúdo da guia
            $tabPanel.append(options.content);
        } else {
            //Sem conteúdo, use o IFRAME para abrir o link
            App.blockUI({
                target: '#tab-content',
                boxed: true,
                message: 'Carregando......'//,
            ...
            var $iframe = $("<iframe></iframe>").attr("src", options.url).css("width", "100%").css("height", "95vh").attr("frameborder", "no").attr("id", "iframe_" + pageId).addClass("tab_iframe").attr(pageIdField, pageId);

            //iframe Carregar evento de conclusão
            $iframe.load(function () {
                App.unblockUI('#tab-content');//Interface de desbloqueio
                App.fixIframeCotent();//Altura corrigida
            });
...
var closeTab = function (item) {
    // item pode ser uma tag ou uma tag
    // Todos eles têm o atributo data-id, e tudo corre bem após a conclusão da aquisição.
    var pageId = getPageId(item);
    closeTabByPageId(pageId);
};

function closeTabByPageId(pageId) {
    var $title = findTabTitle(pageId);//Título da aba
    var $tabPanel = findTabPanel(pageId);// Instalar iframe

    if ($title.hasClass("active")) {
        //A guia a ser fechada está ativa
        //Para passar a classe ativa para outras guias

        //Dar prioridade ao próximo separador, se não, passá-lo ao anterior
        ...
        setTimeout(function () {
            //Algum tipo de bug que precisa ser atrasado
            activeTabByPageId(activePageId);
        }, 100);

    } else {
        //A guia a ser fechada não está ativa
        //Pode ser removido diretamente, sem passar por aula ativa
...
function closeTabOnly(pageId) {
    var $title = findTabTitle(pageId);//Título da aba
    var $tabPanel = findTabPanel(pageId);//Com iframe
    $title.remove();
    $tabPanel.remove();
}
...
function refreshTabById(pageId) {
    ...
    if (url.indexOf(top.document.domain) < 0) {
        $iframe.attr("src", url);// Redefinir o URL em condições de domínio cruzado
    } else {
        $iframe[0].contentWindow.location.reload(true);// Com atualização de parâmetro
    }
...
//Calcular a largura e largura de vários objetos jq
...
//Role até a guia especificada
var scrollToTab = function (element) {
    //elemento é tab (uma tag), com o título
    //div.content-tabs > div.page-tabs-content
    var marginLeftVal = calSumWidth($(element).prevAll()),//A largura total de todas as abas na frente
        marginRightVal = calSumWidth($(element).nextAll());//A largura total de todas as guias atrás
    //一 A largura total dos botões (deslize para a esquerda, direita)
    var tabOuterWidth = calSumWidth($(".content-tabs").children().not(".menuTabs"));
    // Tab (um rótulo) mostra a largura total da área
    var visibleWidth = $(".content-tabs").outerWidth(true) - tabOuterWidth;
    //O comprimento a ser rolado
    var scrollVal = 0;
    if ($(".page-tabs-content").outerWidth() < visibleWidth) {
        //Todas as guias podem ser exibidas
        scrollVal = 0;
    } else if (marginRightVal <= (visibleWidth - $(element).outerWidth(true) - $(element).next().outerWidth(true))) {
        //Role para a direita
        //marginRightVal(A largura total de todas as guias subseqüentes é menor que a área visível - (a largura da guia atual e a próxima guia))
...
//Barra de rolagem rola para a esquerda
...
//Barra de rolagem rola para a direita
...
var closeOtherTabs = function (isAll) {
    if (isAll) {
        ...
        var firstChild = $(".page-tabs-content").children().eq(0); //Verifique o primeiro menu que não pode ser excluído
        if (firstChild) {
            //Ative esta aba
...
$(function () {
    var $tabs = $(".menuTabs");
    //Ative a aba quando você clicar na aba
    $tabs.on("click", ".menu_tab", function () {
        var pageId = getPageId(this);
        activeTabByPageId(pageId);
    });

    //Clique duas vezes na guia para atualizar a página
    ...
    context.init({
        preventDoubleContext: false,//Não desabilite o menu do botão direito do mouse original
        compress: true//Menos preenchimento
    });
...
//            {header: 'Options'},
        {
            ...
            action: function (e, $selector, rightClickEvent) {
                //e é o evento que clica no menu
                //$ selector é ＄（".page-tabs-content")
                //rightClickEvent É o direito de abrir o evento de menu
    ...
    $.fn.sidebarMenu = function (options) {
        ...
        function init($menu_ul, data, level) {
            $.each(data, function (i, item) {
                ...
                var $footer = $('<li class="footermenu"></li>');
                if (item.isFooter !== null && item.isFooter === true) {
                    $footer.append(item.text);
                    $menu_ul.append($footer);
                    return;
                }


                //Se não for um cabeçalho
                ...
                if (item.children && item.children.length > 0) {
                    ...
                    if (item.targetType != null && item.targetType === "blank") //Representante abre uma nova página
                    {
                        $a.attr("href", item.url);
                        $a.attr("target", "_blank");
                    }
                    else if (item.targetType != null && item.targetType === "ajax") { //Abra a página em nome do ajax
                        $a.attr("href", item.url);
                        $a.addClass("ajaxify");
                    }
                    else if (item.targetType != null && item.targetType === "iframe-tab") {
                        item.urlType = item.urlType ? item.urlType : 'relative';
                        var href = 'addTabs({id:\'' + item.id + '\',title: \'' + item.text + '\',close: true,url: \'' + item.url + '?tabid=' + item.id + '\',urlType: \'' + item.urlType + '\'});';
                        $a.attr('onclick', href);
                    }
                    else if (item.targetType != null && item.targetType === "iframe") { //Representa uma única página de iframe
                        $a.attr("href", item.url);
        ...
        //Além disso, o menu de ligação é clicado e outras ações são tomadas.
        $menu_ul.on("click", "li.treeview a", function () {
            var $a = $(this);

            if ($a.next().size() == 0) {//Se tamanho> 0, é considerado expansível.
                if ($(window).width() < $.AdminLTE.options.screenSizes.sm) {//Tela pequena
                    //Acione o evento de clique do botão da barra de menu à esquerda, feche a barra de menus
...
//sidebar - Pacote de componente de menu
//Chame diretamente a barra lateral - método de menu na página
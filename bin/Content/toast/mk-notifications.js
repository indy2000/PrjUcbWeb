...
function mkNotifications(mkConfig){
	...
	var mkNotifications = '<div id="mk-notifications" style="display:none" data-max="'+config.max+'" class="mk-notifications mk-'+config.positionY+' mk-'+config.positionX+scrollable+rtl+'"></div>',
...
function mkNoti(title, message, mkOptions){
	...
	var dismissBtn = $('#mk-close-'+setUniqueId);
    ...
    $(dismissBtn).parent().on('click', function (event) {
        event.stopImmediatePropagation();
        closeMkNoti(setUniqueId);
        if (callback == true) {
            config.callback();

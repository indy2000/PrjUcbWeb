 ...
 */
//Função do Julio (Headers Complexas)
var _fnGetHeaders = function(dt, data) {
  var thRows = $(dt.header()[0]).children();
  var numRows = thRows.length;
  var matrix = [];
  var matrixComplexa = GetMatrixComplexa(dt);
  if (matrixComplexa[0] !== undefined && matrixComplexa[1] !== undefined) {
    matrix[0] = [];
    jafoi = [];

    for (var i = 0; i < data.header.length; i++) {
      var _findIndex1 = matrixComplexa[0].findIndex(function(p, index) {
        return p === data.header[i] && !jafoi.contains(index);
      });

      var _findIndex2 = matrixComplexa[1].findIndex(function(p, index) {
        return p === data.header[i] && !jafoi.contains(index);
      });

      if (_findIndex1 === -1 && _findIndex2 === -1) {
        matrix[0][i] = "";
      } else if (_findIndex1 !== -1) {
        jafoi.push(_findIndex1);

        if (matrixComplexa[0][_findIndex1] && matrixComplexa[1][_findIndex1])
          matrix[0][i] = matrixComplexa[0][_findIndex1];
        else matrix[0][i] = "";
      } else if (_findIndex2 !== -1) {
        jafoi.push(_findIndex2);

        if (matrixComplexa[0][_findIndex2] && matrixComplexa[1][_findIndex2])
          matrix[0][i] = matrixComplexa[0][_findIndex2];
        else matrix[0][i] = "";
      } else {
        matrix[0][i] = "";
      }
    ...
    //Segunda linha do titulo pega do data que é baseado no config de exportação
    matrix[1] = data.header;
    return matrix;
  } else {
    matrix[0] = data.header;
    return matrix; //GetMatrixComplexa(dt);
  }
};

var GetMatrixComplexa = function(dt) {
  var thRows = $(dt.header()[0]).children();
  var numRows = thRows.length;
  var matrix = [];
...
(function(factory) {
  ...
  DataTable.ext.buttons.excelHtml5 = {
    ...
    action: function(e, dt, button, config) {
      ...
      //}
      //Código substituido para atender Headers Complexas (Julio)
      if (config.header) {
        /* ----- BEGIN changed Code ----- */
        var headerMatrix = _fnGetHeaders(dt, data);
        for (var rowIdx = 0; rowIdx < headerMatrix.length; rowIdx++) {
          addRow(
            $.map(headerMatrix[rowIdx], function(d, index) {
              if (!d && index > 1) d = headerMatrix[rowIdx][index - 1];
              return d;
            }),
            rowPos
          );
